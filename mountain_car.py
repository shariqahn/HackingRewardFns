...
Code taken from: https://github.com/HumanCompatibleAI/population-irl/blob/master/pirl/envs/mountain_car.py
"""
...
class ContinuousMountainCarPopulationEnv(Environment):
    metadata = {
    ...
    def __init__(self):
        ...
        self.max_speed = 0.08
        self.power = 0.0017
        self.vel_penalty = 1e-2
        self.initial_noise = 1e-2
        self.goal_reward = np.array([100.])
        ...
        self.viewer = None
        self.target = None # set in randomize_rewards
    ...
    def get_reward(self, state, action, next_state, target=False):
        ...
        left_before = old_position <= self.target
        left_after = position <= self.target
    ...
    def reset(self, rng):
        ...
        trough = rng.randint(0, self.num_peaks - 1)
        noise = rng.uniform(low=-1, high=1) * self.initial_noise
        pos = trough + 0.5 + noise

        self.state = np.array([pos, 0])
        if self.target is not None:
            self.goal_position = self.target
        else:
            self.goal_position = rng.choice([self.min_position + 0.01,
...
if __name__ == "__main__":
    env = ContinuousMountainCarPopulationEnv()
    ...
    for _ in range(500):
        ...
        assert reward == env.get_reward(state, action, next_state)
        state = next_state
